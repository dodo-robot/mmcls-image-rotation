...
get_model_name:
	ls model  | awk 'NR==1 {print $$1}' > model_name.txt

get_minio_pod:
	$(KUBECTL) describe pods -n ${NAMESPACE} | grep Name.*minio | awk 'NR==1 {print $$2}' > minio.txt
	
deploy_models_to_minio:
	MINIO=$(head -n 1 minio.txt)
	MODEL_NAME=$(head -n 1 model_name.txt)
